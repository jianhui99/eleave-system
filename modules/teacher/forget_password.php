...
if(isset($_POST['submit'])){
    ...
    if($email != ""){
        ...
        $query = $con->query($sql);
        if($query-> num_rows > 0){
            $token = "mynameisleongjianhuiandstudentid1830061";
            $token = str_shuffle($token);
            $token = substr($token, 0, 15);

            $sql = "UPDATE teacher SET token = '$token' WHERE email='$email'";
            if($query = $con->query($sql)){
                ...
                try {
                    ...
                    $mail->Body = "
                        ...
                        <a href='http://localhost/E-leaveSystem/modules/teacher/recover_password.php?email=$email&token=$token'>Reset your password.</a>
                    ";
                    $mail->send();
